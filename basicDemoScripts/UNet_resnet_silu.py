...
class ResnetIdentityBlock(tf.keras.Model):
    def __init__(self, filters, kernel_size, res_conv=False, groups=32):
        ...
        self.conv2 = tf.keras.layers.Conv2D(filters2, kernel_size, padding='same',kernel_regularizer=regularizers.L1(0.01))
...
"Fit model"
train_noisy_image = np.load('./noisy.npy')
x_train = np.load('./clean.npy')
"Fit model"
history = autoencoder.fit(train_noisy_image, x_train, epochs=15, shuffle=True, batch_size=2, validation_split = 0.1)
# plot training progress

plot_loss_accuracy(history); plt.show(); plt.savefig('../lossAccuracyPlotUNet_9layer_mae_sigmoid_ebsd.jpg')
compare_loss_accuracy(history); plt.show(); plt.savefig('../compareLossAccuracyPlotUNet_9layer_mae_sigmoid_ebsd.jpg')

# save the trained model
# autoencoder.save('./UNet_9layer_mae_sigmoid_ebsd.h5')
...
plt.savefig('../UNet_9layer_mae_sigmoid_ebsd_accuracy.jpg', pad_inches=0.01)
